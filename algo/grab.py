...
global year_fac
incometable = pd.read_html("https://finance.yahoo.com/quote/3988.HK/financials?p=3988.HK",header=0)
bstable = pd.read_html("https://finance.yahoo.com/quote/3988.HK/balance-sheet?p=3988.HK",header=0)
cashflowtable = pd.read_html("https://finance.yahoo.com/quote/3988.HK/cash-flow?p=3988.HK",header=0)
...
for i in year:
	...
	# X21
	inputdict[i].append(executefunc("totalrev[year_fac]/totalrev[str(float(year_fac)-1)]"))
	# X22
	...
	# X24
	inputdict[i].append(executefunc("(gp[str(float(year_fac)-1)]+gp[str(float(year_fac)-2)]+gp[str(float(year_fac)-3)])/totalasset[year_fac]"))
	# X25
	...
	# X43
	inputdict[i].append(executefunc("(totalrev[year_fac]/((tr[str(float(year_fac)-1)] + tr[year_fac])/2)) + (365/(cogs[year_fac]/((tr[str(float(year_fac)-1)] + tr[year_fac])/2)))"))
	# X44
...
with open("/Users/Lwmformula/Desktop/testdata.pkl","wb") as f:
